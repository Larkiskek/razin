...
def main():
    ...
    while not finished:
        ...
        panda_1 = pygame.Surface((int(x / 3.2), int(y / 2.5)))
        
        draw_panda(panda_1)
        screen.blit(panda_1, (int(1.8 * x / 3), int(y / 2.5)))

        panda_2 = pygame.Surface((int(x / 6), int(y / 5)))
        draw_panda(panda_2)
        screen.blit(panda_2, (int(x / 3), int(y / 2)))
